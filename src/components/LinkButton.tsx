...
const LinkButton: React.FC<LinkButtonProps> = ({ text, url, icon, delay }) => {
  ...
  const renderIcon = () => {
    switch (icon) {
      case 'ai-chatbot':
        return <AiChatbotIcon className="w-5 h-5 text-matrix-green" />;
      case 'habit-tracker':
        return <HabitTrackerIcon className="w-5 h-5 text-matrix-green" />;
      case 'portfolio':
        return (
          <div className="w-5 h-5 flex items-center justify-center text-matrix-green">
  ...
  return (
    <a 
      ...
      className={`matrix-button link-shine animate-matrix-fade stagger-${delay}`}
    >
      {renderIcon()}
